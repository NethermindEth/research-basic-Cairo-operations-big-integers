...
# Takes an integer `num`, a number of bits `n_b`, and a length l, and returns a tuple (d0, ..., d_{l-1})
# of overlapping sections of num of the type used by Uint384_expand
def split2(num: int, num_bits_shift: int = 64, length: int=7) -> List[int]:
    a = []
    num = num << num_bits_shift
    for _ in range(length):
        a.append( num & ((1 << 2*num_bits_shift) - 1) )
        num = num >> num_bits_shift 
    return tuple(a)
